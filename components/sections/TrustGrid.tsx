...
const TRUST_ICONS = [
  ...
  <svg key="3" width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden>
    ...
    <path d="M8 10.5 V8 a4 4 0 0 1 8 0 v2.5" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" />
    <circle cx="12" cy="15" r="1.4" fill="currentColor" />
  </svg>,
  /* spark AI 优势 */
  <svg key="4" width="20" height="20" viewBox="0 0 24 24" fill="none" aria-hidden>
      ...
      <div className="section-container">
        ...
        <div className="mt-10 lg:mt-14 grid grid-cols-1 sm:grid-cols-2 gap-4 lg:gap-6 max-w-4xl mx-auto">
          {dict.items.map((item, i) => {
            const isLast = i === n - 1
            const oddLast = isLast && n % 2 === 1
            return (
              <FadeInX
                key={item.title}
                dir={i % 2 === 0 ? 'left' : 'right'}
                delay={Math.floor(i / 2) * 0.1}
                className={oddLast ? 'sm:col-span-2 sm:max-w-xl sm:mx-auto sm:w-full' : ''}
              >
                <div
                  className={`h-full bg-white rounded-2xl px-6 py-6 border ${
                    isLast ? 'border-sakura/30' : 'border-charcoal/[0.06]'
                  }`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`inline-flex items-center justify-center w-10 h-10 rounded-xl ${ICON_TONES[i % ICON_TONES.length]}`}>

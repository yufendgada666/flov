...
/**
 * 小伴学习机·演示动画：自动循环播放三个例题的完整辅导过程（数学→英语→语文），
 * 悬停/离屏暂停；底部按钮为真机界面样式的静态装饰。全部脚本化，不连真实 AI。
 */
export default function DeviceDemo({ dict, className = '' }: DeviceDemoProps) {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      setScenarioIdx((i) => (i + 1) % dict.scenarios.length)
      setVisibleCount(0)
    }, END_HOLD)
    return () => clearTimeout(t)
  }, [visibleCount, paused, reducedMotion, total, dict.scenarios.length])
      ...
      <div ref={tiltRef} style={{ transform: TILT_REST, transition: 'transform 0.25s ease-out', transformStyle: 'preserve-3d' }}>
        <DeviceView view="front" className="relative" float shadow>
          <div className="absolute inset-0 flex flex-col">
            ...
            {/* 底部两大按钮（真机界面样式，纯装饰） */}
            <div className="flex-shrink-0 flex gap-2 px-2.5 pb-3 pt-1" aria-hidden>
              <div
                className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-[12.5px] font-bold text-white shadow-sm"
                style={{ background: CORAL_GRAD }}
              >
